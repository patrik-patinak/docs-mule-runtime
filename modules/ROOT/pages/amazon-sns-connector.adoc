...
You can install the connector in Anypoint Studio using the instructions in xref:installing-connectors.adoc[Installing Connectors].
...
|*Security Token* |Temporary security credentials which are used to access the AWS resources. 
...
`src` folder of your project. See xref:configuring-properties.adoc[Learn how to configure properties].
...
xref:deploying-to-multiple-environments.adoc[Deploying to Multiple Environments] for instructions on how to manage this.
...
You can download a fully functional example from https://mulesoft.github.io/mule-amazon-sns-connector/[Mule Amazon SNS Connector on github.io].
